...
class Lakalaapi extends Common{
    ...
    */
	public function PaymentTrade(){
        ...
        // 获取用户信息
        $userInfo = Db::name('users')->field('real_name,card_no,bank_no,is_merchant,is_d0,bankbranch_id,phone,is_certificate,card_face_img,card_back_img')->where("series",$series)->find();
        if(!$userInfo){
        ...
        }else if($userInfo['is_certificate'] == 2){
            my_json_encode(10004,'实名认证照片未完善');
        }else{
